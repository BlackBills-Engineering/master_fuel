...
def _bcd_to_int(b: bytes) -> int:
    """
    Packed-BCD → int (старший полубайт идёт первым).
    4 байта дают до 8 цифр.
    """
    val = 0
    for byte in b:
        val = val * 100 + ((byte >> 4) & 0xF) * 10 + (byte & 0xF)
    return val
...
class PumpMaster:
    ...
    async def _parse(self, buf: bytes):
        # драйвер может вернуть несколько кадров, разделённых 0x02
        for chunk in buf.split(b"\x02"):
            if not chunk:
                continue
            fr = b"\x02" + chunk

            # ACK-кадр (6 байт, …03FA) — пропускаем
    ...
    async def _handle_dc(self, addr: int, dc: int, pl: bytes):
        ...
        if dc == 0x01 and len(pl) >= 1:
            ...
            # если колонка перешла в FILLING — регулярно спрашиваем литры/сумму
            if status == PumpStatus.FILLING:
                asyncio.get_running_loop().run_in_executor(None, hw.cd1, addr - 0x50, 0x04)  # RETURN FILLING INFO
            return

        # DC-2 — VOLUME / AMOUNT
        if dc == 0x02 and len(pl) >= 9:
            side_flag = pl[0]                 # 0x00/0x01
            side      = "right" if side_flag & 0x01 else "left"

            vol = _bcd_to_int(pl[1:5]) / 1000   # литры
            amt = _bcd_to_int(pl[5:9]) / 100    # валюта

            s = getattr(p, side)
            s.volume_l, s.amount_cur = vol, amt
            await self.events.put({"addr": addr, "side": side, "volume_l": vol, "amount_cur": amt})
            return

        # DC-3 — NOZZLE IN/OUT
        if dc == 0x03 and len(pl) >= 1:
            noz      = pl[0]
            noz_num  = noz & 0x0F              # 0-15
            side     = "left" if (noz_num & 0x01) == 0 else "right"
            taken    = bool(noz & 0x10)
        ...
        # остальные кадры просто логируем
